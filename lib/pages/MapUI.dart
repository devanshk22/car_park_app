...
class _MapUIState extends State<MapUI> {
  ...
  showMarkers(carparklist) async {
    ...
    for (int i = 0; i < carparklist.length; i++) {
      // double _distanceBetween = (Geolocator.distanceBetween(
      //         _currentLocation.latitude,
      //         _currentLocation.longitude,
      //         carparklist[i].location.latitude,
      ...
      markers.add(Marker(
        ...
        infoWindow: InfoWindow(
          title: carparklist[i].address,
        ),
        onTap: () async {
          Carpark carpark =
              await db.getCarparkByAddress(carparklist[i].address);
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => InfoUI(carpark: carpark)));
        },
      ));
      setState(() {
        markers = markers;
